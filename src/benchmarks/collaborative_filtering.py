if __name__ == '__main__':
  ...
  from src.algorithms import popularity, itemknn, slim, item2vec, matrix_factorization
  ...
  for f in files[2:]:
    ...
    models = [popularity.Popularity(config),
              #matrix_factorization.MFRec(config),
    ...
    for m in models:
      if isinstance(m, slim.SLiMRec):
        ...
        best_params = grid_search(train, m, config, params)
        m.set_params(**best_params)
      # elif isinstance(m, matrix_factorization.MFRec):
      #   params = {
      #     'lr': [0.01],
      #     'reg_1': [0.001],
      #     'reg_2': [0.001], 
