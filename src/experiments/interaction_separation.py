...
from src.algorithms import itemknn, slim, matrix_factorization
...
if __name__ == '__main__':
  ...
  for f in data_files[1:]:
    ...
    df = load_dataset.load_by_filepath(f)
    df = preprocessor.proces(df, k_filter=2) # must filter 2 due to memory restrictions
    ...
    model = matrix_factorization.MFRec(config)
    print('fitting model')
    model.fit(train)
    print('model fitted')
    ...
    filter_num_interactions_df(df, 5)
